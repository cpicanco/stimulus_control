...
function TMatrixForm.GetCircleGrid(AMonitor: integer): TStmCircleGrid;
var
  LP : TPoint;
  LR : TRect;
  LW, LH,
...
procedure TMatrixForm.SetCircleGrid(AStmCircleGrid: TStmCircleGrid);
var
  LCount,LTrial, i : integer;
  LStimulusToGap,
  aGapDegree, aGapLength : integer;
begin
  LStimulusToGap := 0;
  ...
  for LTrial := Low(Trials) to High(Trials) do
    begin
      ...
      Trials[LTrial].Positive := (LTrial mod 2) = 0;
      ...
      for i := Low(AStmCircleGrid) to High(AStmCircleGrid) do
        begin
          ...
          if Trials[LTrial].Positive and (i = LStimulusToGap) then  // there is a gap
            begin
              aGapDegree := 1+Random(360);
              aGapLength := seGapLength.Value;
              Trials[LTrial].Comps[i].Path := '1' + #32 + IntToStr(aGapDegree) + #32 + IntToStr(aGapLength) + #32;
            end
          else // no gap
            Trials[LTrial].Comps[i].Path := '0' + #32 + '1' + #32 + '360' + #32;
        end;

      if Trials[LTrial].Positive then
...
procedure TMatrixForm.btnEditNodes1Click(Sender: TObject);
begin
  case RadioGroupGrids.ItemIndex of
